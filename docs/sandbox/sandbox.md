...
The Sandbox module is consumed by higher-level systems such as the [gateway](../core/gateway.md), [workers](../workers/workers.md), [agent system](../core/shared_core.md), and ABStudio backend. It does not expose user-facing APIs directly; instead, it offers reusable executor classes and helper functions that other modules call.
...
Wraps `DockerExecutor` in an autonomous repair loop. When code fails to compile or run, the engine asks the [model router](../core/shared_core.md) for a fix, strips markdown fences, and retries up to a configured maximum. Used by tool execution, workflow engine, and agent builder.
...
- **[shared_core](../core/shared_core.md)** — provides `compliance_engine` (PCI/PII scanning), `model_router` (LLM routing), and `core.logger`.
- **[llm_proxy](../llm/llm_proxy.md)** — `doc_executor` calls `/llm/imagen` for AI-generated document images.
- **[gateway](../core/gateway.md)** — exposes `sandbox_exec` via `routers/sandbox_router.py`.
- **[workers](../workers/workers.md)** — background workers such as `exec_worker.py` and `doc_worker.py` use sandbox executors.
- **abstudio_backend** — document skills and agent tooling invoke `doc_executor` and `docker_executor`.
...
- The [gateway](../core/gateway.md) receives `sandbox_exec` requests and forwards them to `SelfHealingEngine` or `DockerExecutor`.
- The [workers](../workers/workers.md) use the sandbox to run code jobs, document jobs, and security scans in the background.
- The [agent system](../core/shared_core.md) uses `SelfHealingEngine` for autonomous tool execution and repair.
- ABStudio uses `doc_executor` to render document skills and `docker_executor` for tool execution.

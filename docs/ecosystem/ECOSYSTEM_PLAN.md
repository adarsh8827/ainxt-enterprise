...
Status: DESIGN — no implementation code included, per instruction. All current-state claims below cite `file:line`. All claims about Reference A are cited to an internal reference-design pattern pack (kept outside this repository). Claims about Reference B are marked `[INFERRED — public UX knowledge + an internal UI mock reference, kept outside this repository, only]` since no architecture report equivalent to Reference A's pattern pack was available for Reference B — only a UI mock. **Nothing here proposes deleting or disabling any existing code.** Per explicit instruction, the new Ecosystem system is built **additively alongside** every system below; old tables/routers/UIs keep serving traffic unmodified until an explicit, late-phase, gradual deprecation (§13).
...
**License audit finding, direct action item**: `lucide-react` (ISC) is present in `ai-ui/package.json` and **actively imported in 78 files** across `ai-ui/src/**` (corrected from an earlier count of 77 — see `docs/ecosystem/design/CHANGELOG.md`'s 2026-09-25 entry). The mandate "no lucide (ISC)" therefore implies a **repo-wide icon-library migration**, not just avoiding it in new marketplace UI — scoped explicitly as its own line item in §13, not silently assumed. No lucide dependency exists in `desktop/package.json` or `AgentStudio/frontend/package.json`. Other flags: `jszip` (desktop, dual MIT/GPLv3 — using under MIT is fine, just needs a THIRD-PARTY-NOTICES entry), `docling`/`reportlab` (root `requirements.txt`, reputationally permissive but not yet inline-annotated the way `psycopg2-binary`/`ldap3` already are — recommend a formal license-checker pass). No AGPL/GPL-only packages found. `ldap3` (LGPL) is already correctly isolated to `requirements-ldap.txt`, not the default install — a good existing precedent for how to isolate any future non-MIT/Apache dependency an imported marketplace item might need.

---

## 2. Reference comparison table

| Capability | Reference A (cited) | Reference B `[INFERRED]` | Ours today (cited) | Decision | Reason |
|---|---|---|---|---|---|
| Catalog source model | Curated PR-pointer for code-running types (plugins/MCP); crawled aggregate for skills (Reference A — catalog sources) | `[INFERRED]` centrally curated + community skills directories, browse-then-install | `routers/marketplace_router.py` (in-memory + Redis + one missing JSON file); no DB-backed catalog | **Build**: `ainxt-ecosystem-index` repo, bot-only PRs, auto-merge gate, no human review (§7) | Self-hosted, no central service we run — GitHub Actions is the only "central" compute available |
| Package manifest | SKILL.md (frontmatter+body), `plugin.yaml`, MCP `manifest.yaml` (Reference A — package formats) | `[INFERRED]` SKILL.md-compatible frontmatter | 4 different, unrelated skill formats (System A/B/C/D, §1.1) | **Extend**: adopt agentskills.io SKILL.md as canonical for the *Skills* item type; wrap Plugins/MCP/Connectors in our own pointer schema | Task mandate requires agentskills.io SKILL.md; unifying reduces to one canonical format instead of four |
| Install pipeline | quarantine→scan→trust×verdict policy→move→provenance (Reference A — install pipelines) | `[INFERRED]` install = fetch + local sandbox check | No staging/quarantine step anywhere; `docker_executor.py` scans code at *run* time, not at *install* time | **Build**: staging row (`status=pending_scan`) + `sandbox/docker_executor.py`-based gate before `status=active` (§6) | Direct server-side translation of the pattern; existing sandbox is 80% there |
| Trust × verdict policy | 2 independent axes, one non-overridable floor cell (Reference A — security) | `[INFERRED]` single trust signal (publisher) | None — `ToolSubmission` has a `risk_score` but no policy table | **Adopt** the 2-axis matrix; **hard-code** license failure as pass/block only (task mandate) | Directly portable; matches our stricter "MIT/Apache-2.0 only" requirement even better than Reference A's model |
| Storage model | Files/config for extensions; DB only for conversation state (Reference A — storage model) | `[INFERRED]` server-side DB, content in object storage | 4 skill stores, 2 tool registries, no unified schema (§1.1-1.3) | **Build** real per-tenant Postgres rows + content in object storage, content-hash pinned | Task mandate explicitly requires this; Reference A's file-based model doesn't survive multi-tenant/horizontal scale |
| Installed vs enabled | Two independent facts; disable = config flag, no re-scan (Reference A — installed vs enabled scoping) | `[INFERRED]` similar toggle model | `skills_pg.status`/`is_production` conflates "governed" and "enabled"; no per-surface toggle anywhere | **Adopt**: `ecosystem_installs.enabled` bool, independent of item `status` | Matches existing `is_production` precedent but adds the missing per-surface toggle |
| Chat integration | Progressive disclosure: index in prompt, full body on demand (Reference A — chat integration) | `[INFERRED]` `/command` slash + tool-call fetch, per UI mock | AgentStudio already does this correctly (`skill_manifest.py:131`) for System A only | **Adopt** platform-wide via the unified registry (§4) | Already proven internally at System-A scale; just needs to become the platform-wide default |
| Agent-created items | Closed action-set tool, ledger, staged approval, lower authority for autonomous writes (Reference A — agent-created skills) | `[INFERRED]` "Create with AI" → private draft → publish | `workers/skill_loop_worker.py` already implements exactly this (fail-closed compliance gate, `PENDING_APPROVAL` only) | **Adopt & extend** the existing pattern to Plugins/MCP items | We already have the strongest possible existing precedent — extend, don't reinvent |
| Desktop UI browse | Iframe of public docs site (ADAPT — own it natively instead) (Reference A — desktop UI) | `[INFERRED]` native catalog UI | Desktop is a thin `loadURL` shell over the same `ai-ui` build (§1.6) | **Adopt Reference A's ADAPT verdict**: native React catalog, shared component, no iframe | Desktop already reuses the web UI bundle — trivially gets a native catalog for free |
| Unified tool registry | One registry, 3 origins, registration-time collision policy, session-scoped resolution (Reference A — tool registry) | `[INFERRED]` one tool-call surface, provenance-blind to model | `mcp/registry.py` + `mcp/tool_registry.py` exist but connectors are **not** wired in (dead code, §1.2); 3 separate resolvers (Chat/AgentStudio/Cowork) | **Extend** `mcp/registry.py`/`mcp/tool_registry.py`/`mcp/skill_registry.py` into the single runtime registry for Skills. The dead connector-registration path is a **separate, deferred** concern, gated behind `ECOSYSTEM_CONNECTOR_REGISTRY_BRIDGE` (off) — not required for Skills, not fixed this phase (§15 decision 3) | Task mandate says extend these exact files; Skills don't need the connector-registration path at all, so fixing it isn't a Skills-phase prerequisite |
| MCP lifecycle | Eager/lazy start, circuit breaker + backoff, annotation-based write-gating (Reference A — MCP lifecycle) | `[INFERRED]` managed connection pool | `core/circuit_breaker.py` + `connectors/engine.py` retry logic already implements this for connectors; `mcp/external_registry.py` implements it separately for external MCP | **Adopt as-is**, unify under one lifecycle manager surface | Two good independent implementations exist; unify the *interface*, not necessarily the code |
| Change propagation | Deferred-by-default, pull-based cross-surface sync, cache-prefix stability (Reference A — change propagation/caching) | `[INFERRED]` live push via websocket (per UI mock's real-time feel) | No propagation mechanism exists; three catalogs are each queried fresh per-surface | **Build**: `ecosystem.changed` Redis pub/sub → WebSocket (task mandate requires push, unlike Reference A) | Multi-user org-shared items need push (a teammate disabling something must be seen live) — Reference A never has this problem as a single-user tool |
| Secret scoping/storage | Context-scoped resolution; 3 divergent storage models by credential owner (Reference A — secret scoping/redaction) | `[INFERRED]` per-user OAuth, server-brokered | Context-scoped resolution ✅ already correct (`auth/dependencies.py`); storage is **one static key for everything**, no org_id, no envelope encryption (§1.3) | **Adopt** scoping; **replace** storage with real `SecretStore` (envelope encryption via `core/ckms`, per-tenant DEK) (§8) | This is the single biggest gap found in the current codebase — directly actionable |
...
`ecosystem.changed` events (schema in `CONTRACTS.md`) publish to Redis pub/sub on any install/uninstall/enable/disable/update/block/connection_changed action. A lightweight WebSocket relay (new, small addition to the existing WS infrastructure already used by `routers/cowork_mcp_router.py` and `routers/cowork_dispatch_router.py` for Redis-backed cross-worker notification) pushes to connected web/desktop clients. Chat's next-turn tool list is invalidated via the Redis-cached capability-set hash, matching the "deferred-by-default, explicit-now escape hatch" pattern from reference-design pattern 13 — except our default *is* push (§2), because org-shared items can change under a user who isn't the one who changed them, a scenario Reference A never has to handle.
...
-- 'local' (Review fix 3) is a new kind: exactly one per org, representing "items this org created directly
...
CREATE TABLE ainxt.ecosystem_sources (
    ...
    -- hard FK to the legacy store/credential_vault.py (Review fix 16), for a private-repo access token.
...
CREATE TABLE ainxt.ecosystem_items (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    namespace       TEXT NOT NULL,              -- "publisher/name" — publisher segment MUST resolve in ecosystem_publishers (Review fix 2), enforced at the service layer (not a DB-level FK, since namespace is a composite string, not just the publisher slug)
    ...
    icon_url        TEXT NULL,                  -- format: "url:<same-origin object-storage path>" (raster, or SVG sanitized of <script>/on*/external refs — uploaded via a dedicated endpoint, never an arbitrary external URL, Review fix 7) or "emoji:🧠" (Unicode emoji); NULL → client renders a monogram fallback. Never lucide/an icon-font reference.
    source_id       UUID NOT NULL REFERENCES ainxt.ecosystem_sources(id),   -- always NOT NULL — user/org-created items point at that org's 'local' source row rather than allowing NULL (Review fix 3 decision)
...
-- Namespace uniqueness (Review fix 2): global across the shared catalog scopes, but scoped per-org for
...
-- Endpoints: PUT/DELETE /ecosystem/featured/{item_id} (Review fix 12).
...
-- Ephemeral Create-with-AI conversational drafts (Review fix 10) — backed by the AgentStudio Skill
...
-- `surfaces` has NO hardcoded default (Review fix 5) — the service layer always sets it explicitly from
...
CREATE TABLE ainxt.ecosystem_installs (
    ...
    origin          TEXT NOT NULL CHECK (origin IN ('created','shared','provisioned','required','added')) DEFAULT 'added',   -- powers the "Yours" grouping (CONTRACTS.md §9's Install schema, Review fix 9)
    ...
    enabled         BOOLEAN NOT NULL DEFAULT TRUE,   -- independent of item status — reference-design pattern 7
...
CREATE TABLE ainxt.ecosystem_credentials (
    ...
    issuer          TEXT NULL,                  -- OAuth issuer binding, per reference-design pattern 12/14
...
-- `visible_item_types` vs `enabled_item_types` (Review fix 18): visible = shown as a tab at all (all 4
...
-- Org product entitlement (Review fix 1) — which product(s) an org is allowed to use, and which is the
...
Full typed contract lives in `CONTRACTS.md` — **`CONTRACTS.md` §17 is the authoritative, consolidated endpoint list** (supersedes the outline that used to live inline here, which drifted out of sync once the Review's fixes added drafts/icon-upload/delete-draft/featured endpoints). Summary of the shape, not a duplicate list:
...
6. **Ethics & policy** — LLM-assisted review by a **separate reviewer model with fresh context** (new call to `models/model_router.py`, not reusing whatever model authored/reviewed the item): harmful/deceptive/discriminatory intent, behavior-vs-description mismatch, over-broad triggers, excessive permissions. **If the reviewer model is unavailable** (call fails, times out, or is unreachable), this stage's verdict is `pending` with a scheduled retry — never `pass` by default. An inability to review is not itself a finding, but it is also never treated as an implicit pass; this applies uniformly, including to first-party/builtin seeding (M0-review fix, `docs/ecosystem/design/CHANGELOG.md`).
7. **MCP/connector checks** — HTTPS-only, OAuth 2.x for authenticated services, every tool has a title + `readOnlyHint`/`destructiveHint` (ambiguous → treated as destructive, per reference-design pattern 12's fail-safe rule), namespace/domain ownership, live `tools/list` + smoke test per tool (reuse `connectors/probe.py`'s cheapest-safe-tool-selection strategy as the smoke-test picker), requested OAuth scopes declared and minimal.
...
- `crawl.yml` — scheduled, discovers items in approved sources (Phase 1: our own GitHub repos with MIT/Apache items + the official MCP Registry API only), opens auto-PRs. Modeled on Reference A's tiered-CI pattern (Reference A — CI pipelines): cheap structural check on every PR, expensive clone+checkout+run only on the diff.
...
**Sources, Phase 1 default-ON**: our own GitHub repos with per-item-verified MIT/Apache license, plus the official MCP Registry API. **Default-OFF, admin-addable**: GitHub taps, `.well-known/skills` endpoints, private org repos (token via `ecosystem_sources`'s own SecretStore-shaped columns — `secret_backend`/`credential_ciphertext`/`credential_dek_key_id`/`credential_external_ref`, §4 — a seam this phase, not `store/credential_vault.py`, per Review fix 16). **Excluded per mandate**: ClawHub. **ToS-gated before crawl**: LobeHub, browse.sh (`ecosystem_sources.tos_checked_at` must be set). Self-hosted instances never scrape third-party marketplaces directly — only the central index (or an admin-approved source) does that, mirroring Reference A's own "browsing is cheap, crawling is centralized" principle (Reference A — catalog sources) but replacing "one desktop app's own crawl" with "our GitHub Actions crawl."

**Per-source implementation status (updated task I, pre-M3)**: `github_repo` (single-repo, on-demand import — SKILL.md pinned to a resolved commit SHA, repo SPDX + SKILL.md `license:` both independently checked) and `well_known` (`/.well-known/agent-skills/index.json` schema 0.2.0, falling back to `/.well-known/skills/index.json`; sha256-verified on every fetch) both landed as real, tested adapters this task — see `LLD/external-import.md`. **Moved to the MCP phase (task 3 in `SKILLS_PHASE_PLAN.md`'s "Next phases"), not built yet**:

- **The official MCP Registry API** (`https://registry.modelcontextprotocol.io`, API v0.1). Specifics for whoever picks this up:
  - **External-publisher namespace mapping**: `mcpreg:<reverse-dns name>` (e.g. a registry entry named `io.github.acme/tool` maps to `mcpreg:io.github.acme.tool` in `ecosystem_items.namespace`) — keeps registry-sourced items in their own namespace band, never colliding with a locally-created `acme/tool`.
  - **Icon re-hosting**: the registry entry's own icon URL is never stored directly in `ecosystem_items.icon_url` (would violate the existing `ICON_SOURCE_NOT_ALLOWED` rule, task B-7) — fetch once, sanitize/store through `icon_service.py` exactly like a user upload, then reference the re-hosted `url:` form.
  - **License from package metadata**: an MCP Registry entry itself carries no license field — resolve it from the underlying npm (`package.json license`) or PyPI (`Classifier :: License ::`/`license` metadata field) package the entry points at. Missing/unresolvable → blocked, same `LICENSE_NOT_ALLOWED` treatment as every other adapter.
  - **OCI-packaged servers blocked unless the image carries an explicit MIT/Apache-2.0 license label** (`org.opencontainers.image.licenses`) — no license field to check otherwise means no import, not a permissive default.
  - **ToS path for remote-only servers** (no downloadable package, HTTP/SSE endpoint only): record the ToS acknowledgement on the `ecosystem_sources` row the same way `github_repo`/`well_known` do (`get_or_create_import_source`'s pattern), before ever connecting.
  - **Live `tools/list` only after connect** — the registry entry's own metadata never substitutes for actually calling the MCP server's `tools/list`; the gate's `mcp_connector_stage` (still an inert no-op for skills, B-9) is where this phase wires that check in.
  - **Curated subset, not the full registry** — mirrors this section's own "browsing is cheap, crawling is centralized" principle; an admin approves specific registry entries rather than every self-hosted instance crawling the entire registry independently.
- **skills.sh** — its own API is **not usable for this platform**: it requires a Vercel OIDC token, which a self-hosted instance has no way to obtain. Import skills.sh-listed skills via their underlying GitHub repos instead — the `github_repo` adapter already built this task handles that transparently (skills.sh is a discovery layer over GitHub-hosted content, not a separate fetch protocol).

**Per-type split** (mirrors Reference A's Skills-vs-Plugins/MCP split, Reference A — catalog sources): Skills = crawled index (lower trust ceiling, cheap to review); Plugins and MCP = curated pointer files with pinned SHAs (higher trust ceiling, code-running); Connectors = built-in `connector_definitions`-shaped entries plus cards that resolve to a plugin/MCP install — no hosted connector portal, each self-hosted instance is its own credential broker (§8).
...
1. **Platform/app secrets** — one per instance (KEK reference, DB creds, OAuth client IDs/secrets the org registered). Stored in env vars or `oauth_provider_configs`'s own SecretStore-shaped columns (`secret_backend`/`client_id_ciphertext`/`client_id_dek_key_id`/`client_id_external_ref`, §4 — a seam this phase, per Review fix 16, not a `credential_vault` FK).
...
MCP-over-HTTP reuses the MCP SDK's OAuth client provider (license-checked before use, per the hard licensing constraint) with a custom token-storage backend on the new `SecretStore`, keyed `(org_id, user_id, server_name)` — **not** `server_name` alone, closing the multi-tenant token-collision risk Reference A's own pattern-pack analysis calls out (Reference A — secret scoping/redaction's server-side translation note). MCP sessions pooled per `(user, server)`, never shared across users (matching reference-design pattern 12's translation guidance, and consistent with `routers/cowork_mcp_router.py`'s existing per-user Redis session routing pattern, `:15-26,48-52`).

Skill scripts in the sandbox get **no** raw tokens — a script needing an API calls a broker proxy endpoint with a short-lived, scoped capability token (one connector, allowed actions, minutes). Local/stdio MCP on the server side: secrets injected only into that sandboxed subprocess's env at spawn, per user — mirroring the existing `served_profile_child_env()`-style pattern Reference A documents (not present in ainxt today, a genuinely new control to add, since `mcp/external_registry.py`'s stdio launch currently has no per-user env isolation described in the research).
...
- **Self-hosting**: each instance registers its own OAuth apps (redirect URI = its own domain). Admin setup wizard per provider shows the exact redirect URI, accepts client ID/secret (encrypted directly into `oauth_provider_configs`'s own SecretStore-shaped columns — a seam this phase, not `credential_vault`, per Review fix 16), runs a test call.
...
Copyright/license text (+ NOTICE for Apache-2.0) stored in `ecosystem_item_versions.attribution`, shown on the item's detail page License tab. Each external source requires a ToS check recorded (`ecosystem_sources.tos_checked_at`) before crawl; a takedown path exists via `ecosystem_sources.enabled=false` + a re-scan/hide sweep. External secret backends (Cloud KMS, Vault) are called over their APIs only, never bundled — their client SDKs still pass the license check like any other dependency. Icons: no lucide (ISC) — enforced first in the new `ecosystem-ui` package (§9); the pre-existing 78-file usage is a separately tracked migration (§13), not silently grandfathered into the license gate's allowlist forever.

### 11.1 License policy — explicit, for everything (Review round following M1)

The five-point list above is the original design; this subsection makes the same rule fully explicit and precise, since "the license rule applies" had left room to wonder whether it covered a bundled icon or a transitive npm dependency as clearly as it covers the item's own declared `license` field. It does not change the rule — it closes every gap in stating it.

**Scope — applies to all 4 item types (skills, plugins, connectors, MCP servers) and everything they contain or pull in**, with no exception by content kind: the item's own code/instructions, any adapter or SDK it bundles, every transitive dependency at its pinned version (pip/npm), icons, fonts, images, and templates. There is no "it's just an asset, not code" carve-out — an MIT-licensed skill bundling a GPL-licensed icon fails the gate exactly as it would for a GPL-licensed dependency.

**Allowed licenses**: MIT, Apache-2.0. A dual/multi-license declaration is acceptable **only if at least one option is MIT or Apache-2.0**, and the specific option elected must be recorded (matching the existing `jszip: MIT OR GPL-3.0-or-later, used under MIT` pattern already in `THIRD-PARTY-NOTICES.md`). Everything else — GPL/AGPL/LGPL/BSD-only/ISC/MPL/CC-\*/none/unclear/mixed/source-available/non-commercial — is blocked automatically. **No admin override exists for this check anywhere** (this was already true for the item's own license field per point 3 above; it is now stated to be equally true for every dependency, adapter, SDK, icon, font, image, and template the item brings with it).

**Enforced at exactly 3 points, each independent** (a content author or a repo contributor cannot bypass one by only satisfying another):
1. **Import pre-check** (task B-6, M2) — before any content is downloaded or fetched, the declared license metadata (a manifest field, a `package.json`/`requirements.txt` line, a repo's own `LICENSE` file reference) is checked and a disallowed declaration is rejected with `LICENSE_NOT_ALLOWED` **before a single byte of the actual content is retrieved**. This is a metadata-only check — it catches the common case cheaply and early, but is not a substitute for point 2, since declared metadata can be wrong or absent.
2. **Gate stage 2** (task B-8, M2) — full SPDX detection on the item's own declared license *and* every dependency in its resolved tree (not just direct dependencies — a pinned version can itself depend on something disallowed). A Software Bill of Materials (SBOM) — every dependency name, pinned version, and detected license — is generated and **stored alongside the version** (`ecosystem_item_versions.manifest` gains a stored SBOM structure, or a sibling column if the shape doesn't fit cleanly — task B-8 decides the exact column at implementation time) so a later audit never has to re-derive what was actually shipped in a given version. This stage alone can never be overridden by `--force` or admin action, matching point 2 of the original five-point list above.
3. **CI** (extends task B-21's existing check) — for anything that lives in *this* repository: builtin/optional shipped items (`ecosystem/builtin/`, `ecosystem/optional/`), any first-party adapter code, and `packages/ecosystem-ui` itself. Same MIT/Apache-2.0-only rule, same no-override policy, enforced before merge rather than at runtime install time.

**Remote/vendor-hosted MCP servers or APIs — license check does not apply, ToS review does.** When nothing is downloaded or executed by this platform (a remote MCP server we only call over HTTPS, a vendor API), there is no code or asset of theirs to license-check — instead, a **recorded source ToS review** is required before the server/API can be added as a source (mirrors the existing `ecosystem_sources.tos_checked_at` mechanism, §7). Any client SDK *we* install to talk to that remote service (e.g. an MCP SDK's HTTP transport, a vendor's Python/JS client library) is a normal dependency of our own code and **is** license-checked like any other, at all 3 enforcement points above — the remote service itself being exempt does not exempt the library we chose to depend on to reach it.

**Tests, one set per enforcement point** (land with the task that implements each point): point 1 — a fixture import with a disallowed declared license is rejected before any fetch is attempted (task B-6); point 2 — a fixture item with an allowed top-level license but a disallowed transitive dependency is blocked, and the resulting SBOM correctly lists every dependency's detected license (task B-8); point 3 — a fixture PR adding a disallowed-license asset (not just a disallowed import) anywhere under `ecosystem/builtin/`, `ecosystem/optional/`, or `packages/ecosystem-ui/` fails CI (extends task B-21's existing test suite).
...
**This section's week-by-week table is superseded by `SKILLS_PHASE_PLAN.md`'s milestone breakdown (M0-M5)** as the actual execution order — that document is the one being implemented against and updated task-by-task; this table is kept below as historical planning context (the reasoning behind sequencing decisions, the legacy-deprecation ordering, the Phase-1.5 stretch-goal status) rather than being treated as a literal schedule going forward. Where the two disagree on ordering, `SKILLS_PHASE_PLAN.md`'s milestones win.
...
6. **`lucide-react` repo-wide usage (§1.6/§11) — RESOLVED**: banned in new code; existing 78-file usage goes on a CI allowlist with a separately-filed cleanup ticket.

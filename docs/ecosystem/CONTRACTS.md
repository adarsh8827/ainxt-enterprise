...
- **import**: `application/json` — `{ create_via: "import", kind: "github_repo" | "well_known" | ..., ref: "owner/repo[@branch_or_sha]" | "domain/skill_slug", item_type, namespace }` — fetched, validated, and gated exactly like any other creation path; no special trust conferred by having come from a URL. **Updated at task I (pre-M3)**: `kind` values match `ecosystem_sources.kind`'s real DB constraint (`ECOSYSTEM_PLAN.md` §4 — `github_repo`/`mcp_registry`/`well_known`/`private_git`/`skills_sh_indirect`/`local`), not the earlier draft's `"url"`/`"github"` placeholders. `github_repo`/`well_known` discover and verify their own license from the source content itself (repo SPDX + SKILL.md `license:` field, or the index entry's `license` field respectively — `LLD/external-import.md`) — the top-level `license` field is not used/required for these two kinds specifically; every other, not-yet-implemented `kind` value still requires the caller-declared `license` field for its pre-check.

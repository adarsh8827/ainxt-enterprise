...
1. **Marketplace naming + nested routes** (resolves M1, M6): the nav entry, route prefix, and all copy use **"Marketplace"** (not "Customize" — the FE's existing name wins, since it's already shipped and users would see it first). Routes become a real tree: `/marketplace/:typeSlug`, `/marketplace/:typeSlug/new`, `/marketplace/:typeSlug/upload`, `/marketplace/:typeSlug/import` (§9C fix 11 — the third create-payload path, `create_via: import`, missing from an earlier revision of this list), `/marketplace/:typeSlug/:namespace` — using the route-slug mapping from §2 above, so `/marketplace/mcp/:namespace` is correct even though the enum value is `mcp_server`.

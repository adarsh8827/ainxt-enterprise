...
- **Security follow-up, recorded not fixed (item 2, pre-M3): `docker-compose.yml`'s `gateway` service has its own, pre-existing `/var/run/docker.sock` mount, predating this initiative** (doc-generation pipeline's sandbox health check + `sandbox/docker_executor.py`'s SDK-path optimization — see that service's own comment). The Docker socket is root-equivalent host access: any process that can reach it can launch a privileged container and escape its own confinement, regardless of what that process's own code intends to use it for. This initiative's own gate sandbox never runs there (`sandbox/ecosystem_gate_executor.py`'s fail-closed `ECOSYSTEM_GATE_SANDBOX_ALLOWED` check, `LLD/gate.md`) — but the mount itself remains, for a reason unrelated to and out of scope for this initiative to change (removing it would be an out-of-scope edit to the pre-existing, unrelated doc-generation feature, per the standing additive-only rule). Whoever eventually revisits the doc-generation pipeline's own sandbox architecture should treat this as a known, tracked risk to close — e.g. by moving that health check/SDK-path optimization to run inside `doc-worker` (which already has its own `docker.sock` mount for the actual sandbox execution) instead of the gateway.

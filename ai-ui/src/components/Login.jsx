...
// ─── Google sign-in ───────────────────────────────────────────────────────────
// Reasons GET /auth/google/callback can bounce the browser back here as
// ?auth_error=<code>. Keep in sync with auth/google_auth.py.
const GOOGLE_ERRORS = {
  google_denied:            "Google sign-in was cancelled.",
  google_bad_request:       "Google sign-in failed. Please try again.",
  google_state_expired:     "That sign-in attempt expired. Please try again.",
  google_state_invalid:     "That sign-in attempt was not valid. Please try again.",
  google_exchange_failed:   "Could not complete Google sign-in. Please try again.",
  google_no_identity:       "Google did not return an identity. Please try again.",
  google_invalid_token:     "Google returned an invalid response. Please try again.",
  google_no_email:          "Your Google account did not share an email address.",
  google_email_unverified:  "Your Google account email address is not verified.",
  google_user_error:        "Could not sign you in. Please try again.",
  not_registered:           "No AiNxt account exists for that Google address. Contact your administrator.",
  account_disabled:         "That account has been disabled.",
};

// Google's official "G" mark. Their sign-in branding guidelines require this
// artwork rather than a generic icon; it is inlined instead of shipped as an
// asset so the repo carries no third-party image file.
...
// A full-page navigation, deliberately not a fetch(): the backend 302s to
// Google and the callback lands back here with the httpOnly cookie already set,
// which the SPA then picks up via /auth/me. Nothing to handle client-side.
function GoogleSignInButton({ dm, label }) {
  return (
    <>
      <div className="flex items-center gap-3 my-4">
        <span className={`h-px flex-1 ${dm ? 'bg-white/10' : 'bg-gray-200'}`} />
        <span className={`text-[11px] ${dm ? 'text-gray-600' : 'text-gray-400'}`}>or</span>
        <span className={`h-px flex-1 ${dm ? 'bg-white/10' : 'bg-gray-200'}`} />
      </div>
      <button
        type="button"
        onClick={() => { window.location.href = `${API}/auth/google/login`; }}
        className={`w-full py-3 rounded-xl border text-sm font-semibold transition-all duration-200 hover:scale-[1.02] flex items-center justify-center gap-2.5 cursor-pointer ${
          dm
...
// Mirrors core/security_validation.py::password_strength_errors — same four
// rules, same order, so a client-side rejection reads identically to what the
// server would say anyway. Used wherever a NEW password is chosen (register,
// forced password reset) — never for the login form's existing-password
// field, which must accept whatever the account's real password already is.
...
function passwordStrengthErrors(password) {
  ...
  if (!PASSWORD_SPECIAL_RE.test(password))       errors.push("Password must contain at least one special character");
  return errors;
}
...
export default function Login({ onAuth, forcePasswordReset = false, onPasswordResetComplete = () => {} }) {
  ...
  const [view,        setView]        = useState("login"); // "login" | "register" | "forgot" | "reset-required"

  // A banner shown on the LOGIN view only (e.g. "password updated, sign in
  // again") — distinct from `error`, which is red/failure styled.
  const [loginNotice, setLoginNotice] = useState("");

  // ── Forced password reset (temp-password login) ──────────────────────────
  const [resetTempPassword,    setResetTempPassword]    = useState("");
  const [resetNewPassword,     setResetNewPassword]     = useState("");
  const [resetConfirmPassword, setResetConfirmPassword] = useState("");
  const [resetError,           setResetError]           = useState("");
  const [resetLoading,         setResetLoading]         = useState(false);
  const [showResetTemp,        setShowResetTemp]        = useState(false);
  const [showResetNew,         setShowResetNew]         = useState(false);
  const [showResetCfm,         setShowResetCfm]         = useState(false);

  // Mount-time / mid-flow-refresh case: App.jsx already confirmed (via
  // /auth/me) that this session's password is still a temp one. Asking again
  // for the temp password here — rather than trying to carry it over from a
  // login submit that may not have happened in this page load at all — is
  // what makes this work identically whether we just logged in or the page
  // was simply refreshed while this screen was already showing.
  useEffect(() => {
    if (forcePasswordReset) setView("reset-required");
  }, [forcePasswordReset]);
  ...
  // Only ever populated when the server operator has opted into
  // SHOW_TEMP_PASSWORD_IN_UI (core/config.py); cleared on every view switch
  // so a stale password can never linger on screen.
  ...
  const [uiConfig, setUiConfig] = useState({ internal_use_only: false, self_registration_enabled: true, forgot_password_enabled: true, smtp_configured: false, platform_name: "AiNxt", pii_payload_encryption_enabled: false, google_login_enabled: false });

  // ── Google sign-in failure surfaced via ?auth_error= ─────────────────────
  // The callback is a top-level browser redirect, so a failure cannot be
  // returned as a fetch response — it comes back as a query param instead.
  const [oauthError, setOauthError] = useState("");
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const code = params.get("auth_error");
    if (!code) return;
    setOauthError(GOOGLE_ERRORS[code] || "Sign-in failed. Please try again.");
    // Drop the param so a refresh does not replay a stale message.
  ...
  async function submit(e) {
    e.preventDefault();
    setError(""); setBlocked(false); setLoginNotice("");
    ...
    try {
      ...
      // A temp password (forgot-password flow) is a fully valid password —
      // login above already succeeded and a real session now exists. Do NOT
      // call onAuth() (that would grant entry to the app); instead hold here
      // on the "set new password" view. Pre-fill the temp password they just
      // typed so they are not asked to retype it a moment later.
      if (meData.is_temp_password) {
        setResetTempPassword(identifier);
        setView("reset-required");
        return;
      }

      // Defensive: a normal (non-temp) login just succeeded, so any stale
      // forcePasswordReset left over from an earlier, abandoned temp-password
      // session — e.g. logged out before finishing the reset — must not be
      // able to poison a later mount. See the matching fix in
      // App.jsx::handleLogout for the actual bug this papers over.
  ...
  // ── Forced password reset submit ──────────────────────────────────────────
  // Reuses POST /auth/change-password as-is (no backend change): it already
  // requires a valid session, verifies current_password against the stored
  // hash, and clears is_temp_password on success. The only thing specific to
  // this screen is what happens AFTER: rather than continuing straight into
  // the app, the session is deliberately logged out and the user is sent back
  // to a plain sign-in with their new password — an explicit extra step, not
  // an oversight.
  async function submitPasswordReset(e) {
    e.preventDefault();
    setResetError("");

    const tempVal    = resetTempPassword;
    const newVal     = resetNewPassword;
    const confirmVal = resetConfirmPassword;

    if (!tempVal || !newVal || !confirmVal) { setResetError("All fields are required"); return; }
    const _resetPwErrors = passwordStrengthErrors(newVal);
    if (_resetPwErrors.length)               { setResetError(_resetPwErrors[0]); return; }
    if (newVal !== confirmVal)               { setResetError("New password and confirmation do not match"); return; }
    if (newVal === tempVal)                  { setResetError("New password must be different from your temporary password"); return; }
    ...
    try {
      const res  = await authFetch(`${API}/auth/change-password`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ current_password: tempVal, new_password: newVal }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setResetError(data.detail || "Could not update your password");
        return;
      }

      // Deliberately end the session and require a fresh, normal sign-in
      // with the password just set — rather than letting this same session
      // continue into the app.
      try { await authFetch(`${API}/auth/logout`, { method: "POST" }); } catch { /* proceed regardless */ }

      onPasswordResetComplete();
      setResetTempPassword(""); setResetNewPassword(""); setResetConfirmPassword("");
      setIdentifier("");
      // The captcha challenge/answer from the temp-password login attempt
      // that led here must not carry over — otherwise the sign-in form
      // reappears with someone else's (well, their own, but stale) captcha
      // answer already sitting in the box.
      setCaptchaInput(""); captchaInputRef.current = ""; setCaptchaError("");
      setCaptchaText(genCaptchaText());
      setLoginNotice("Your password has been updated. Please sign in with your new password.");
      setView("login");
    } catch {
      setResetError("Cannot connect to server. Is the backend running?");
    } finally {
  ...
  async function submitRegister(e) {
    ...
    const _regPwErrors = passwordStrengthErrors(regPassword);
    if (_regPwErrors.length)        { setRegError(_regPwErrors[0]); return; }
  ...
  async function submitForgot(e) {
    ...
    try {
      ...
      // temp_password is only ever non-null when the server operator has
      // opted into SHOW_TEMP_PASSWORD_IN_UI (core/config.py) AND SMTP is not
      // configured — see the security note on that flag before assuming this
      // is safe to enable on a deployment reachable by untrusted users.
      let data = null;
      try { data = await res.json(); } catch { /* fall through to text messaging below */ }

      if (data?.temp_password) {
        // No separate banner text here — the password card below carries its
        // own instruction, and showing both was redundant and threw off the
        // layout. forgotTempPassword itself now satisfies every "success has
        // happened" guard below (email field, submit button, back-to-sign-in).
        setForgotTempPassword(data.temp_password);
      } else if (uiConfig.smtp_configured) {
        // Sign-in with a temp password now always leads to the forced
        // "set a new password" screen automatically — not a manual trip to
        // Profile → Security, which is what this used to (incorrectly) say.
        setForgotSuccess("A temporary password has been sent to your email. Sign in with it — you'll be asked to set a new password.");
      } else {
        setForgotSuccess("SMTP is not configured, so your temporary password was printed to the server console. Ask your administrator to check the gateway logs (docker compose logs gateway). It replaces any earlier temporary password. Sign in with it — you'll be asked to set a new password.");
      }
      setForgotEmail("");
    } catch {
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-10 fade-in-up" style={{ animationDelay: '0s' }}>
              <div className="w-9 h-9 flex items-center justify-center">
                <BrandMark plated className="w-9 h-9 shadow-lg shadow-indigo-900/30" />
              </div>
              <div>
        ...
        <div className="flex-1 flex items-center justify-center relative">
          ...
          <div className="fade-in-up w-full max-w-sm px-6 relative z-10" style={{ animationDelay: '0.15s' }}>
            ...
            <div className="lg:hidden flex items-center gap-2 mb-8 justify-center">
              <div className="w-8 h-8 flex items-center justify-center">
                <BrandMark plated className="w-8 h-8 shadow-md shadow-indigo-900/25" />
              </div>
            ...
            {view === "forgot" ? (
              ...
              <>
                ...
                <form onSubmit={submitForgot} className="space-y-4" autoComplete="off">
                  ...
                  {/* Email — hidden once a result (banner or temp password)
                      is showing, matching the submit button below. Left
                      visible before that, this was still an editable input
                      sitting above the result, which is what threw the whole
                      card's alignment off. */}
                  {!(forgotSuccess || forgotTempPassword) && (
                    <div>
                  ...
                  {/* Success banner — only when there is no password card
                      below to show instead; having both said the same thing
                      twice and made the card feel unbalanced. */}
                  {forgotSuccess && !forgotTempPassword && (
                  ...
                  {/* Temp password — only when the server operator has opted
                      into SHOW_TEMP_PASSWORD_IN_UI (core/config.py). Shown
                      once; there is no way to retrieve it again after this
                      screen is left. One compact block: label, password,
                      instruction — no separate banner duplicating it. */}
                  {forgotTempPassword && (
                    <div className={`rounded-xl px-4 py-3 text-center ${dm ? 'bg-white/5 border border-white/10' : 'bg-gray-50 border border-gray-200'}`}>
                      <div className={`text-[11px] mb-1 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>
                        Temporary password
                      </div>
                      <div className={`text-lg font-bold tracking-widest font-mono ${dm ? 'text-white' : 'text-gray-900'}`}>
                        {forgotTempPassword}
                      </div>
                      <div className={`text-[11px] mt-1.5 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>
                        Sign in with this — you'll be asked to set a new password.
                      </div>
                    </div>
                  )}

                  {/* Submit — hidden once a result is showing */}
                  {!(forgotSuccess || forgotTempPassword) && (
                    <button
                      type="submit"
                  ...
                  {/* Back to login once a result is showing */}
                  {(forgotSuccess || forgotTempPassword) && (
                    <button
                      type="button"
                      onClick={switchToLogin}
              ...
              <>
                ...
                <form onSubmit={submitRegister} className="space-y-4" autoComplete="off">
                  ...
                  <div>
                    ...
                    <p className={`text-[11px] mt-1.5 ml-1 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>
                  ...
                  <div>
                    ...
                    {/* Always rendered so this row's height is constant —
                        see the matching note on the reset-required view. */}
                    <p className="text-red-400 text-[11px] mt-1.5 ml-1 h-4">
                      {regConfirm && regPassword !== regConfirm ? "Passwords do not match" : " "}
                    </p>
                  </div>
                  ...
                  {/* Submit — disabled while every field is still empty */}
                  <button
                    type="submit"
                    disabled={
                      regLoading ||
                      (!regName.trim() && !regEmail.trim() && !regPassword && !regConfirm) ||
                ...
                {uiConfig.google_login_enabled && (
                  <GoogleSignInButton dm={dm} label="Sign up with Google" />
                )}
              ...
              /* ── Forced password reset — temp password login ───────────
                 No "back to sign in" here: this step is mandatory, not
                 optional — the session is real (this is the same account),
                 but the account may not proceed until a new password is set. */
              <>
                {/* Single heading — no subtitle. "Temporary password" as the
                    first field's own label already says why this screen is
                    here; a second explanatory line on top of it was redundant. */}
                <h2 className={`text-xl font-semibold mb-6 ${dm ? 'text-white' : 'text-gray-900'}`}>Set a new password</h2>

                <form onSubmit={submitPasswordReset} className="space-y-4" autoComplete="off">

                  {/* Temporary password */}
                  <div>
                    <label className={`block text-xs font-medium mb-1.5 ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Temporary password</label>
                    <div className="relative">
                      <input
                        type={showResetTemp ? "text" : "password"}
                        value={resetTempPassword}
                        onChange={e => setResetTempPassword(e.target.value)}
                        placeholder="The temporary password you just used"
                        autoComplete="off"
                        name="ainxt-reset-temp"
                        className={`${dm ? 'input-glow-dark bg-white/5 border-white/10 text-white placeholder-gray-600' : 'input-glow-light bg-gray-50 border-gray-200 text-gray-900 placeholder-gray-400'} w-full border rounded-xl px-4 py-3 pr-11 text-sm outline-none transition-all duration-200`}
                      />
                      <button type="button" onClick={() => setShowResetTemp(p => !p)}
                        className={`absolute right-3 top-1/2 -translate-y-1/2 transition ${dm ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'}`}>
                        {showResetTemp ? <EyeOff size={15} /> : <Eye size={15} />}
                      </button>
                    </div>
                  </div>
                  ...
                  {/* New password */}
                  <div>
                    <label className={`block text-xs font-medium mb-1.5 ${dm ? 'text-gray-400' : 'text-gray-500'}`}>New password</label>
                    <div className="relative">
                      <input
                        ...
                        autoComplete="new-password"
                        name="ainxt-reset-new"
                        className={`${dm ? 'input-glow-dark bg-white/5 border-white/10 text-white placeholder-gray-600' : 'input-glow-light bg-gray-50 border-gray-200 text-gray-900 placeholder-gray-400'} w-full border rounded-xl px-4 py-3 pr-11 text-sm outline-none transition-all duration-200`}
                      />
                      <button type="button" onClick={() => setShowResetNew(p => !p)}
                        className={`absolute right-3 top-1/2 -translate-y-1/2 transition ${dm ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'}`}>
                  ...
                  <div>
                    <label className={`block text-xs font-medium mb-1.5 ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Confirm new password</label>
                    <div className="relative">
                      <input
                        type={showResetCfm ? "text" : "password"}
                        value={resetConfirmPassword}
                        onChange={e => setResetConfirmPassword(e.target.value)}
                        placeholder="Re-enter your new password"
                        autoComplete="new-password"
                        name="ainxt-reset-confirm"
                        className={`${dm ? 'input-glow-dark bg-white/5 border-white/10 text-white placeholder-gray-600' : 'input-glow-light bg-gray-50 border-gray-200 text-gray-900 placeholder-gray-400'} w-full border rounded-xl px-4 py-3 pr-11 text-sm outline-none transition-all duration-200`}
                      />
                      <button type="button" onClick={() => setShowResetCfm(p => !p)}
                        className={`absolute right-3 top-1/2 -translate-y-1/2 transition ${dm ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'}`}>
                        {showResetCfm ? <EyeOff size={15} /> : <Eye size={15} />}
                      </button>
                    </div>
                    {/* Always rendered (never mount/unmount) so this row's height
                        is constant — a conditionally-rendered hint made the form
                        grow/shrink under the button as the user typed. */}
                    <p className="text-red-400 text-[11px] mt-1.5 ml-1 h-4">
                      {resetConfirmPassword && resetNewPassword !== resetConfirmPassword ? "Passwords do not match" : " "}
                    </p>
                  </div>

                  {/* Error — resetError never duplicates the mismatch hint above:
                      the submit button is disabled whenever the two fields
                      differ, so that branch of submitPasswordReset can only be
                      reached by a same-value/backend rejection, not a mismatch. */}
                  {resetError && (
                    <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-xl px-4 py-3">
                      {resetError}
                    </div>
                  ...
                  {/* Submit — this screen needs ALL THREE fields filled before
                      it's even worth trying (unlike the other forms, which only
                      block a fully-blank submit): a click with one or two
                      fields still empty is guaranteed to fail server-side, and
                      repeated failed attempts here count against
                      AUTH_LOGIN-style rate limits, not just a validation retry. */}
                  <button
                    type="submit"
                    disabled={
                      resetLoading ||
                      !resetTempPassword || !resetNewPassword || !resetConfirmPassword ||
              ...
              {oauthError && (
                <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-xl px-4 py-3 mb-4">
                  {oauthError}
                </div>
              )}

              {loginNotice && (
                <div className="bg-green-500/10 border border-green-500/20 text-green-400 text-xs rounded-xl px-4 py-3 mb-4">
                  {loginNotice}
                </div>
              )}
          ...
          */}
              <form onSubmit={submit} className="space-y-4" autoComplete="off">
                ...
                <button
                  type="submit"
                  disabled={loading || (!email.trim() && !identifier)}
                  className="btn-cta w-full !mt-4 py-3 rounded-xl brand-grad-r hover:from-blue-500 hover:to-indigo-500 text-white text-sm font-semibold transition-all duration-200 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
                >
              ...
              {uiConfig.google_login_enabled && (
                <GoogleSignInButton dm={dm} label="Continue with Google" />
              )}

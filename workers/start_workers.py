...
# Also add AgentStudio/backend so RQ workers can import `app.*` — mirrors the
...
_ABS_BACKEND = os.path.join(_REPO_ROOT, "AgentStudio", "backend")
...
def _cron_scheduler_thread(stop_event: threading.Event):
    ...
    interval_jobs = [
        ...
        # Recover index_requests stuck in 'running' whose worker process was
        # killed/crashed outright — same technique as codewiki_stale_recovery
        # (checks RQ's own job registry, not a fixed time threshold, since
        # indexing legitimately runs from seconds to 10+ hours). Without
        # this, a crashed job's per-repo Redis lock (up to 12h TTL) silently
        # blocks every re-submission of that repo until it expires — see
        # workers/index_worker.py's recover_stale_index_locks().
        ("index_stale_recovery", 10 * 60, "workers.index_worker",      "recover_stale_index_locks"),

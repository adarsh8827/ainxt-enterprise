...
def can_approve_governance(current_user: dict) -> bool:
    """Non-raising "senior approver" check for the flat-mode-aware product /
    KB-document / codebase-index approval gates.

    HOD_APPROVAL_ENABLED=true  (legacy HOD-hierarchy mode, unchanged) —
      delegates to can_approve() (ad_level <= APPROVAL_AD_LEVEL, or admin).
    HOD_APPROVAL_ENABLED=false (flat/admin-only mode, default) — only an
      admin counts as a senior-approver fallback; a non-admin senior
      employee (ad_level <= APPROVAL_AD_LEVEL) can no longer approve these
      three flows, since there's no department/HOD hierarchy to have
      granted them that authority. Flat-mode deployments are assumed to have
      exactly one admin, so that admin's own submission auto-approves (see
      is_sole_active_admin) rather than needing a second admin's review.
    """
    from core.config import HOD_APPROVAL_ENABLED
    if HOD_APPROVAL_ENABLED:
        return can_approve(current_user)
...
def is_sole_active_admin(current_user: dict) -> bool:
    """Non-raising check, used by the flat-mode (HOD_APPROVAL_ENABLED=false)
    approval gates for product creation, KB document upload, and
    codebase-index submission.

    Deliberately kept as a simple `is_admin()` alias: an admin's own
    product/KB/codebase submission always auto-approves in flat mode,
    regardless of how many other admins exist — self-approval for these
    three flows is intentional, not a bug, so this does NOT do a real
    "only one admin exists" DB count.

    Budget-increase requests are a separate flow with its own dedicated
    routing (see store.budget_store.resolve_budget_approvers_for_request).
    In flat mode, an admin's own budget-increase request never goes through
    the approval queue at all — it's applied immediately, the same as
    /budget/hod/self-increase (see routers/budget_router.py::request_increase).
    Only a NON-admin user's request fans out to every active admin for
    approval; whichever admin acts first wins and every other admin's
    pending row is superseded/rejected.
    """
    return is_admin(current_user)
